...
@app.route('/au2', methods=['GET'])
def check_card():
    lista = request.args.get('lista')
    if not lista:
        return jsonify({"status": "error", "response": "No card provided (lista=cc|mm|yy|cvv)"}), 400

    try:
        cc, mm, yy, cvv = lista.split('|')
        cc = cc.replace(" ", "")
        if len(yy) == 2:
            yy = "20" + yy
    except Exception:
        return jsonify({"status": "error", "response": "Invalid format"}), 400
    ...
    try:
        ...
        if not m:
            return jsonify({"status": "error", "response": "Nonce not found (Site/Proxy Issue)"}), 200
        ...
        if 'wordpress_logged_in_' not in str(scraper.cookies):
            return jsonify({"status": "error", "response": "Registration failed (Check proxy/IP)"}), 200

        # print("💳 Tokenizing card with Stripe...")
        p = scraper.get(u + "payment-methods/", timeout=30)
        ...
        if pm.status_code != 200:
            return jsonify({"status": "error", "response": f"Stripe Error: {pm.text[:100]}"}), 200
        ...
        return jsonify({"status": status, "response": msg}), 200
